...
- [主な機能](#主な機能)
- [現在の運用仕様](#現在の運用仕様)
- [配信ユーザーの登録について](#配信ユーザーの登録 or 配信停止)
- [セットアップ](#セットアップ)
...
配信ユーザーを新規登録するには、あらかじめアプリ管理者が`toggle_survey_subscription_trigger.ts`から配信設定用のShortcut Triggerを作成します。

```zsh
slack trigger create --trigger-def triggers/toggle_survey_subscription_trigger.ts
```

作成されたURLを利用者本人が実行すると、そのSlackユーザーが定期配信の対象として登録されます。同じURLは複数の利用者で共有できるため、利用者ごとにトリガーを作成する必要はありません。登録済みの利用者が同じURLを再度実行すると、定期配信は停止されます。実行後は、現在の配信状態が本人宛てのDMで通知されます。
